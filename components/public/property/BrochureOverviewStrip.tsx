...
export function BrochureOverviewStrip({
  ...
  return (
    <section id="stay-details" className="bg-white border-b border-charcoal/10">
      <div className="max-w-screen-xl mx-auto px-6 md:px-12 py-8 md:py-10">
        {facts.length > 0 && (
          <FadeUp>
            <div
              className="grid grid-cols-2 gap-x-6 gap-y-6 sm:grid-cols-3 lg:grid-cols-[repeat(var(--facts),minmax(0,1fr))] lg:gap-x-0 lg:divide-x lg:divide-charcoal/10"
              style={{ "--facts": String(facts.length) } as CSSProperties}
            >
              {facts.map((fact) => (
                <div key={fact.label} className="min-w-0 lg:px-6 lg:first:pl-0 lg:last:pr-0">
                  <p className="font-display font-normal text-xl md:text-2xl leading-none tracking-[-0.01em] text-charcoal">
                    {fact.value}
                  </p>
                  <p className="mt-2 font-sans text-[10px] uppercase tracking-[0.2em] text-charcoal/50">
                    {fact.label}
                  </p>
                </div>
              ))}
        ...
        {/* House Notes — a slim hairline note row under the facts. */}
        {houseNotes.length > 0 && (
          <FadeUp
            delay={0.08}
            className={`flex flex-wrap items-baseline gap-y-2 ${
              facts.length > 0 ? "mt-6 border-t border-charcoal/10 pt-5" : ""
            }`}
          >
            <span className="mr-6 font-sans text-[10px] uppercase tracking-[0.24em] font-bold text-charcoal/45">
              House Notes
            </span>
            {houseNotes.map((note, i) => (
              <span key={i} className="flex items-baseline">
                {i > 0 && <span className="mx-4 h-3 w-px self-center bg-charcoal/15" aria-hidden />}
                <span className="font-sans text-[13px] leading-relaxed font-light text-charcoal/60 whitespace-pre-line">
                  {note}
                </span>
              </span>
            ))}
